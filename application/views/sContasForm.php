			...
			<div class="panel-body p-2 mt-4">
				<div class="row">
					<div class="col-lg-4 col-sm-12">
						...
						<select name="tipo_conta" class="form-control border-0" id="tipo_conta">
							<option <?php if ($tipo == null) {
								echo 'selected';
							} else {
								echo "";
							} ?> disabled> -- Selecione --
							</option>
							<option value="p" <?php if ($tipo == 'p') {
								echo "selected";
							} else {
								echo "";
							} ?>> Saída
							</option> <!-- Pagar -->
							<option value="r" <?php if ($tipo == 'r') {
								echo "selected";
							} else {
								echo "";
							} ?>> Entrada
					...
					<div class="col-lg-4 col-sm-12">
						<label class="m-0 mt-2 labelVencimento" for="" <?php if ($tipo != 'p') {
						...
						<div class="input-group">
							<input type="hidden" id="data_vencimento_original" value="<?= $vencimento ?>">

							<input type="text" name="vencimento" class="form-control border-0 data datepicker-dmy"
								   id="vencimento" placeholder="dd/mm/aaaa" value="<?= $vencimento ?>">
							<div class="input-group-prepend rounded-right text-dark border-0">
								<div class="input-group-text rounded-right text-dark border-0" id="showPassword"
					...
					<div class="col-lg-4 col-sm-12" id="divcontafixa">
						...
						<select name="conta_fixa" class="form-control border-0" id="conta_fixa">
							<option value=" " <?php if ($contaFixa == null) {
								echo 'selected="selected"';
							} else {
								echo "";
							} ?> disabled>-- Selecionar --
							</option>
							<option value="s" <?php if ($contaFixa == 's') {
								echo 'selected="selected"';
							} else {
								echo "";
							} ?>> Sim
							</option>
							<option value="n" <?php if ($contaFixa == 'n') {
								echo 'selected="selected"';
							} else {
								echo "";
							} ?>> Não
				...
				<div class="row">
					<div class="col">
						...
						<input type="text" class="form-control border-0" name="nome_conta" id="nome_conta"
							   placeholder="Ex.: Salário, Conta de Luz, Internet" value="<?= $descricao ?>">
					</div>
				</div>
				<div class="row">
					...
					<div class="col-lg-6 col-sm-12 mt-2" id="divparcelamento" <?php if ($tipo == null or $tipo == "r") {
						echo 'style="display: none;"';
					} ?>>
						<div class="row">
							<div class="col-lg-12 col-sm-12" id="subDivParcelamento">
								...
								<select name="tipo_parcela" class="form-control border-0" id="tipo_parcela">
									<option class="" value=" " <?php if ($tipoParcela == null) {
										echo 'selected="selected"';
									} else {
										echo "";
									} ?> disabled> -- Selecione --
									</option>
									<option class="" value="v" <?php if ($tipoParcela == "v") {
										echo 'selected="selected"';
									} else {
										echo "";
									} ?>> A Vista
									</option>
									<option class="" value="p" <?php if ($tipoParcela == "p") {
										echo 'selected="selected"';
									} else {
										echo "";
									} ?>> A Prazo
							...
							<div class="col-lg-5 col-sm-12"
								 id="div_tipo_parcelamento" <?php if ($tipoParcela == null or $tipoParcela == "v") {
								echo 'style="display: none;"';
							} ?>>
								<label class="m-0 labelParcelamento" for="">Parcelamento:</label>
								<div class="input-group">
									<input type="text" class="form-control border-0" name="parcelamento"
										   id="parcelamento" placeholder="Ex.:12" maxlength="3"
										   onkeypress="return number(event)" value="<?= $parcelamento ?>">
									<div class="input-group-append">
					...
					<div class="col-lg-3 col-sm-12" id="divStatus" <?php if ($contaFixa == null or $contaFixa == "s") {
						echo 'style="display: none;"';
					} ?>>
						...
						<select name="status" class="form-control border-0" id="status">
							...
							<option value="n" <?php if ($status == 'n' or $status == null) {
								echo 'selected';
							} else {
								echo "";
							} ?>> Não
				...
				<div class="row" <?php if ($dataPagamento == null or $tipo == 'r' or $dataPagamento == '0000-00-00 00:00:00') {
					echo 'style="display: none;"';
				} ?>>
					<div class="col col-sm-12">
						<label class="m-0 mt-2 labelStatus" for="">Data e Hora do Pagamento:</label>
						<input type="text" disabled class="form-control border-0" name="dataPagamento"
							   id="dataPagamento" onkeypress="return number(event)"
							   value="<?= date('d/m/Y H:i:s', strtotime($dataPagamento)) ?>">
					</div>
				</div>
...
<!-- MODAL PARA EXCLUSÃO -->
<div class="modal fade" id="excluirConta" data-backdrop="static" data-keyboard="false" tabindex="-1"
...
<div class="modal fade" id="alterarConta" data-backdrop="static" data-keyboard="false" tabindex="-1"
	 aria-labelledby="staticBackdropLabel" aria-hidden="true">
	<div class="modal-dialog modal-lg modal-dialog-centered">
		<div class="modal-content corpo_modal">
			<div class="modal-header">
